...
class DataPlayerWidgetView:
    def __init__(self, player, max_idx):
        ...
        self.record_button = widgets.ToggleButton(
            value=False, description="Record", icon="microphone"
        )
        self.record_output = widgets.Text(
            value="record.wav",
            description="Output path:",
        )
        self.record_overwrite = widgets.Checkbox(value=False, description="Overwrite")

        self.export_button = widgets.Button(description="Export", icon="level-down")
        self.export_output = widgets.Text(
            value="out",
            description="Output path:",
        )
        self.export_format = widgets.Dropdown(options=["WAV", "AIFF"], value="WAV")
    ...
    def _toggle_record(self, value):
        if value["new"]:
            self._player.record_start(
                self.record_output.value, overwrite=self.record_overwrite.value
            )
        else:
    ...
    def _on_export(self, button):
        self._player.export(
            self.export_output.value, header_format=self.export_format.value
        )
...
class RTDataPlayerWidgetView:
    def __init__(self, player):
        ...
        self._record_overwrite = widgets.Checkbox(value=False, description="Overwrite")

        # data logger controls
        log = widgets.ToggleButton(value=False, description="Log", icon="save")
        self._log_output = widgets.Text(
            value="log.csv",
            description="Output path:",
        )
        self._log_overwrite = widgets.Checkbox(value=False, description="Overwrite")
    ...
    def _toggle_record(self, value):
        if value["new"]:
            self._player.record_start(
                self._record_output.value, overwrite=self._record_overwrite.value
            )
        else:
            self._player.record_stop()
    ...
    def _toggle_log(self, value):
        if value["new"]:
            self._player.log_start(
                self._log_output.value, overwrite=self._log_overwrite.value
            )
        else:
...
class RTDataPlayerMultiWidgetView(RTDataPlayerWidgetView):
    def __init__(self, player):
        ...
        for stream in player.streams:
            ...
            log_overwrite = widgets.Checkbox(value=False, description="Overwrite")
